...
- (void)viewDidLoad
{
    ...
    BalancedColumnLayout *layout = [[BalancedColumnLayout alloc] init];
...
{
	UICollectionViewCell * cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"BalancedCell" forIndexPath:indexPath];
	...
	if ([self.imageCache objectForKey:imageURL] == nil)
	{
		...
		dispatch_async(queue, ^{
            UIImage *image = [UIImage imageNamed:@"garching"];
...
- (CGFloat)collectionView:(UICollectionView *)collectionView layout:(BalancedColumnLayout*)collectionViewLayout heightForHeaderInSection:(NSInteger)section
{
	return 50.0;
}
...
- (CGFloat)collectionView:(UICollectionView *)collectionView layout:(BalancedColumnLayout*)collectionViewLayout heightForFooterInSection:(NSInteger)section
{
	return 25.0;
}
